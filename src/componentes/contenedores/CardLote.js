...
const CardLote = ({ lote, esMiSucursal = false }) => {
    ...
    return (
        <>
            <style>{`
                .lc-card { border-radius: 14px; border: 1.5px solid #e2e8f0; background: #fff; overflow: hidden; transition: box-shadow .2s, transform .15s; height: 100%; display: flex; flex-direction: column; }
                .lc-card:not(.inactivo):not(.otra-sucursal):hover { box-shadow: 0 8px 28px rgba(0,0,0,.1); transform: translateY(-2px); }
                .lc-card.inactivo { opacity: .7; filter: grayscale(30%); }
                .lc-card.otra-sucursal { border-color: #fde68a; background: #fffbeb; }
                .lc-img-wrap { position: relative; height: 180px; background: #f8fafc; overflow: hidden; flex-shrink: 0; }
                ...
                .lc-badge-estado { position: absolute; top: 8px; left: 8px; z-index: 5; font-size: .62rem; font-weight: 700; padding: 2px 8px; border-radius: 99px; letter-spacing: .04em; box-shadow: 0 2px 6px rgba(0,0,0,.12); }
                .lc-badge-sucursal { position: absolute; top: 8px; right: 8px; z-index: 5; font-size: .62rem; font-weight: 700; padding: 2px 8px; border-radius: 99px; background: #fde68a; color: #92400e; }
