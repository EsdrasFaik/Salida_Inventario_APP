...
const GuardarLote = () => {
    ...
    useEffect(() => {
        const tieneEspacios = /\s/.test(formulario.numeroLote);
        setErrorNumeroLote(
            (formulario.numeroLote.length > 0 && formulario.numeroLote.trim().length < 2) || tieneEspacios
        );
    }, [formulario]);
    ...
    useEffect(() => {
        setErrorNumeroLote(
            formulario.numeroLote.length > 0 && formulario.numeroLote.length < 2
        );
    }, [formulario.numeroLote]);
                                ...
                                <div className="col-md-6 col-sm-12">
                                    <div className="form-group">
                                        <label htmlFor="numeroLote">
                                            Número de lote {esEdicion && <i className="fas fa-lock ml-1" title="Campo bloqueado en edición" />}
                                        </label>
                                        <input
                                            type="text"
                                            id="numeroLote"
                                            name="numeroLote"
                                            className={`form-control ${errorNumeroLote ? "is-invalid" : ""} ${esEdicion ? "bg-light" : ""}`}
                                            placeholder="Ej: LOT2024001 (Sin espacios)"
                                            value={formulario.numeroLote}
                                            onChange={manejador}
                                            readOnly={esEdicion}
                                        />
                                        {esEdicion ? (
                                            <small className="text-muted">El número de lote único no se puede modificar.</small>
                                        ) : (
                                            errorNumeroLote && <div className="invalid-feedback">Ingrese un número válido (mín. 2 caracteres, sin espacios).</div>
                                        )}
                                    </div>
                                </div>
                                <div className="col-md-6 col-sm-12">
